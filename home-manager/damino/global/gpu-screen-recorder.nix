  ...
  hdr-to-sdr = pkgs.writeShellScriptBin "hdr-to-sdr" ''
    ...
    while (( attempt <= max_retries )); do
      ...
      if ${pkgs.coreutils}/bin/nice -n 18 ${pkgs.ffmpeg-full}/bin/ffmpeg -y \
        ...
        -i "$input" \
        -vf "libplacebo=tonemapping=spline:gamma=1.2:contrast=1.00:colorspace=bt709:color_primaries=bt709:color_trc=bt709:range=full,format=p010,hwupload" \
    ...
    if ! $success; then
      ...
      if ${pkgs.coreutils}/bin/nice -n 18 ${pkgs.ffmpeg-full}/bin/ffmpeg -y -init_hw_device vulkan \
        -i "$input" \
        -vf "hwupload,libplacebo=tonemapping=spline:gamma=1.2:contrast=1.00:colorspace=bt709:color_primaries=bt709:color_trc=bt709:range=full,hwdownload,format=yuv420p10" \
