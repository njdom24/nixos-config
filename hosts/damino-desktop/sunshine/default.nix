    ...
    sunshine = {
      ...
      gamescopeConfig = pkgs.writeShellScript "gamescopeConfig" ''
        if ${pkgs.procps}/bin/pgrep -f ".gamescope-wrapped" > /dev/null || \
          ...
          ${pkgs.systemd}/bin/systemd-run --user --unit=sunshine-steam --remain-after-exit --description="Launch Steam Gamescope detached in desktop session" ${pkgs.bash}/bin/bash -c 'gsc-watcher -e -- steam -tenfoot -pipewire-dmabuf -console -cef-force-gpu'
        fi
      '';
      in
      [
